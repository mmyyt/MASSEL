...
<link rel="stylesheet" type="text/css" href="/resources/css/userBuyerDemandForm.css">
<link rel="stylesheet"
        ...
        <div class="centerBox">
			...
			<c:choose>
				...
				<div class="sellerFormList">
					...
					<c:choose>
						<c:when test="${list.dsvo.disDel == 0 }">
						<a href="/demandSurvey/demandBuyerFormDetail?dno=${list.dsvo.dno }">
					 	<div class="sellerSaleForm">
							<div class="sellerSaleFormImg">
							
								<div class="statusDiv">
								<c:choose>
									<c:when test="${list.dsvo.status == 'before' }">
										시작전
									</c:when>
									<c:when test="${list.dsvo.status == 'ongoing' }">
										진행중
									</c:when>
									<c:when test="${list.dsvo.status == 'ended' }">
										종료됨
									</c:when>
								</c:choose>									
							...
							src="/upload/demandSurveyImg/${fn: replace(list.dsivo.saveDir, '\\','/')}/${list.dsivo.uuid }_${list.dsivo.fileName}"></div>
							<div class="saleBox">
								<div>진행여부</div>
								<div class="saleFormTitle">${list.dsvo.dtitle }</div>
								<div class="saleFormRegDate">참여일 ${list.dsppvo.participationDate }</div>
							</div>
							<div class="saleBox2">
								<div>
								</div>
						...
						<c:when test="${list.dsvo.disDel == 1 }">
						<a href="/demandSurvey/demandBuyerFormDetail?dno=${list.dsvo.dno }">
					 	<div class="sellerSaleForm">
							<div class="sellerSaleFormImg">
